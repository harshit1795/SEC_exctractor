...
class _FinQChatTabState extends ConsumerState<FinQChatTab> {
  ...
  void initState() {
    ...
    // Sessions are loaded after first frame so we can safely read Riverpod
  ...
  /// Returns the Firebase UID of the signed-in user, or 'anonymous' as fallback.
  String get _currentUserId {
    // Synchronous auth check so we don't rely on the stream having emitted yet.
  ...
  Future<void> _fetchSessions() async {
    ...
    try {
      ...
      final response = await apiClient.get('/chat/sessions?user_id=$userId');
  ...
  Future<void> _sendMessage() async {
    ...
    try {
      final apiClient = ref.read(apiClientProvider);
      // Ensure user_id is always present in context_data so the backend
      // associates the new session with the correct Firebase user.
      final contextWithUser = {
        ..._contextData,
